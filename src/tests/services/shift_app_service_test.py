...
class TestShiftAppService(unittest.TestCase):
    ...
    def test_login_with_valid_credentials(self):
        user = self.shift_app_service.login(self.user.username, self.user.password)
    ...
    def test_create_shift_inavlid_information(self):
        with pytest.raises(InvalidShiftInformation):
            self.shift_app_service.create_shift(date="",time="5:00-12:00", location="Kumpula", employee=None)
    
    def test_create_user_with_too_short_username(self):
        with pytest.raises(UsernameTooShortError):
            self.shift_app_service.create_user(username="a", password="Password1!", role="Employee")
        
    def test_create_user_with_no_role(self):
        with pytest.raises(NoRoleError):
            self.shift_app_service.create_user(username="Simo", password="Password1!", role="")
    
    def test_create_user_with_taken_username(self):
        with pytest.raises(UsernameExistsError):
            self.shift_app_service.create_user(username="Samuli", password="Password1!", role="Employee")
    
    def test_create_user_with_invalid_password_no_special_characters(self):
        with pytest.raises(InvalidPassword):
            self.shift_app_service.create_user(username="Simo", password="Password1", role="Employee")
    
    def test_create_user_with_invalid_password_no_numbers(self):
        with pytest.raises(InvalidPassword):
            self.shift_app_service.create_user(username="Simo", password="Password!", role="Employee")

    def test_create_user_with_invalid_password_too_short(self):
        with pytest.raises(InvalidPassword):
            self.shift_app_service.create_user(username="Simo", password="Pas1!", role="Employee")
